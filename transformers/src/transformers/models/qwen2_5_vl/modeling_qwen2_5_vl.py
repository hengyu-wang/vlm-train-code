...
# Prompt (ID 1) 的 token: 只能关注在它之前的 Prompt token。
# Thought (ID 2, 3, ...) 的 token: 只能关注在它之前的、属于同一个Thought的token，以及所有的Prompt token。
# Summary (ID 10) 的 token: 可以关注在它之前的所有token（包括Prompt、所有Thoughts，以及之前的Summary token）。
...
class Qwen2_5_VLAttention(nn.Module):
    ...
    def __init__(self, config: Qwen2_5_VLTextConfig, layer_idx: Optional[int] = None):
        ...
        # parathinker添加的segment embedding
...
class Qwen2_5_VLDecoderLayer(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        position_embeddings: Optional[tuple[torch.Tensor, torch.Tensor]] = None,
        seg_ids: Optional[torch.Tensor] = None,   #新增代码
        **kwargs: Unpack[FlashAttentionKwargs],
...
class Qwen2_5_VLTextModel(Qwen2_5_VLPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # It may already have been prepared by e.g. `generate`
        if not isinstance(causal_mask_mapping := attention_mask, dict):
            # +++ 新增逻辑: 如果提供了 seg_ids，就使用我们的特殊 mask +++
            if seg_ids is not None:
